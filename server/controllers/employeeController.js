...
exports.getEmployeesSalary = async (req, res) => {
    try {
        const employees = await employeeModel.getEmployeeSalaries();
        if (!employees) {
            res.writeHead(404, { 'Content-Type': 'application/json' });
            return res.end(JSON.stringify({ message: 'Employees not found' }));
        }
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify(employees));
    } catch (err) {
        ...
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ message: 'Error fetching employees' }));
    }
};
